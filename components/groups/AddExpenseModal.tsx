...
export default function AddExpenseModal({
  ...
  useEffect(() => {
    ...
    setAmountInput('');
    setCurrency(defaultCurrency || currencyOptions[0] || 'EUR');
    setPaidByParticipantId(activeParticipants[0]?.id ?? '');
    setSplitMode('equal');
    setSelectedParticipantIds(
      activeParticipants.map((participant) => participant.id)
    );
    setPercentageByParticipantId(buildEqualPercentageMap(activeParticipants));
  }, [open, defaultCurrency, currencyOptions, activeParticipants]);
